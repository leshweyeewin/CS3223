...
public class RandomOptimizer2{
	...
	public Operator getOptimizedPlan(){
		...
		Operator S0 = rip.prepareInitialPlan();
		Operator S = S0;
		modifySchema(S);
		Operator bestPlan = S;
		double temperature = getInitialTemperature(S0);
		...
		while(!isFrozen(temperature, countOfBestPlanNotChanging))
		{
			...
			while(equilibrium > 0)
			{
				...
				int bestPlanCost = new PlanCost().getCost(bestPlan);
				int initPlanCost = new PlanCost().getCost(S);
