...
public class RandomOptimizer2{
	...
	modifySchema(initialPlan);
	Operator bestPlan = initialPlan;
	double temperature = getInitialTemperature(initialPlan);
	...
	while(!isFrozen(temperature, countOfBestPlanNotChanging))
	{
		while(equilibrium > 0)
		{
			Operator neighborPlan = getNeighbor(initialPlan);
			System.out.println("------------------neighbor--------------");
			Debug.PPrint(neighborPlan);
			int neighborCost = pc.getCost(neighborPlan);
			System.out.println(neighborCost);
			int costDifference = neighborCost - pc.getCost(initialPlan);
			
			if(costDifference <= 0)
			{
				initialPlan = neighborPlan;
			}
			if(costDifference > 0)
			{
				double probability = Math.exp(-1.0 * (costDifference/temperature));
				
				//Accept uphill with prob
			...
			int bestPlanCost = pc.getCost(bestPlan);
			int initPlanCost = pc.getCost(initialPlan);
			if(initPlanCost < bestPlanCost)
			{
				bestPlan = initialPlan;
				modifySchema(bestPlan);
				System.out.println("------------------new best plan--------------");
				Debug.PPrint(bestPlan);
				int bestCost = pc.getCost(bestPlan);
	...
	if(node.getOpType()==OpType.JOIN){
	    ...
	    case JoinType.BLOCKNESTED:
	    	BlockNestedLoop bn = new BlockNestedLoop((Join)node);
		    bn.setLeft(left);
		    bn.setRight(right);
		    bn.setNumBuff(numbuff);
			return bn;
